...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    event_bus.subscribe(vec!["tts_started", "tts_completed"], |envelope| {
...
async fn demo_tts(event_bus: &Arc<EventBus>) -> Result<(), Box<dyn std::error::Error>> {
    ...
    tts.speak("Hello! This is LUNA's text to speech system.").await?;
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
    
    // Message kinds with profiles
    println!("▶ Critical message (high priority)");
    tts.speak_with(MessageKind::Critical, "Alert! This is a critical message.").await?;
    tokio::time::sleep(tokio::time::Duration::from_secs(2)).await;
...
async fn demo_context() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(stats) = ctx.get_stats("volume up") {
        println!("   'volume up' - Count: {}, Success rate: {:.1}%",
                 stats.count, stats.success_rate() * 100.0);
    }
...
async fn demo_health_aware_tts(event_bus: &Arc<EventBus>) -> Result<(), Box<dyn std::error::Error>> {
    println!("\n═══════════════════════════════════════");
