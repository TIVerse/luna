...
use luna::os::{common, discovery, OsInterface};
...
async fn main() -> anyhow::Result<()> {
    ...
    match discovery::discover_applications().await {
        Ok(apps) => {
            ...
            let editors = apps
                .iter()
                .filter(|a| a.category == AppCategory::TextEditor)
                .count();
            let terminals = apps
                .iter()
            ...
            let media = apps
                .iter()
                .filter(|a| a.category == AppCategory::Media)
                .count();
            let comm = apps
                .iter()
                .filter(|a| a.category == AppCategory::Communication)
                .count();
            let office = apps
                .iter()
                .filter(|a| a.category == AppCategory::Office)
                .count();
            let games = apps
                .iter()
                .filter(|a| a.category == AppCategory::Games)
                .count();
            let system = apps
                .iter()
                .filter(|a| a.category == AppCategory::System)
                .count();
            let other = apps
                .iter()
                .filter(|a| a.category == AppCategory::Other)
                .count();

            println!("📊 Applications by Category:");
            if browsers > 0 {
                println!("   Browser: {}", browsers);
            }
            if ides > 0 {
                println!("   IDE: {}", ides);
            }
            if editors > 0 {
                println!("   TextEditor: {}", editors);
            }
            if terminals > 0 {
                println!("   Terminal: {}", terminals);
            }
            if media > 0 {
                println!("   Media: {}", media);
            }
            ...
            let ides: Vec<_> = apps
                .iter()
                .filter(|a| a.category == AppCategory::IDE || a.category == AppCategory::TextEditor)
                .take(5)
                .collect();
            ...
            for term in search_terms {
                let matches: Vec<_> = apps.iter().filter(|a| a.matches(term)).take(3).collect();
