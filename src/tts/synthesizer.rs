...
use crate::tts::engine::{TtsEngine, OsTtsEngine, VoiceInfo};
use crate::tts::ssml::{parse_ssml, chunk_text, strip_ssml_tags};
...
impl TextToSpeech {
    ...
    pub fn new() -> Result<Self> {
        ...
        info!("✅ TTS synthesizer initialized with engine: {}", engine.name());
    ...
    pub fn apply_profile(&mut self, profile: &VoiceProfile) -> Result<()> {
        debug!("Applying voice profile: rate={}, pitch={}, volume={}", 
    ...
    pub async fn speak_chunked(&mut self, text: &str, max_chunk_len: usize) -> Result<()> {
        ...
        for (i, chunk) in chunks.iter().enumerate() {
            debug!("Speaking chunk {}/{}: {}", i + 1, chunks.len(), 
