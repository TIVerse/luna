...
impl TtsEngine for OsTtsEngine {
    async fn speak(&mut self, text: &str, interrupt: bool) -> Result<()> {
        ...
        let result = self.tts.speak(text, interrupt)
    ...
    fn set_voice(&mut self, voice_id: &str) -> Result<()> {
        ...
        let target = voices.iter()
            .find(|v| v.id() == voice_id || v.name().to_lowercase().contains(&voice_id.to_lowercase()));
    ...
    fn voices(&self) -> Result<Vec<VoiceInfo>> {
        ...
        Ok(voices.iter().map(|v| VoiceInfo {
            id: v.id().to_string(),
