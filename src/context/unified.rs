...
impl CommandStats {
    ...
    pub fn update(&mut self, success: bool, confidence: f32) {
        ...
        if self.count == 1 {
            ...
            self.avg_confidence = (self.avg_confidence * (self.count - 1) as f32 + confidence) / self.count as f32;
        }
    }
}
...
impl UnifiedContext {
    ...
    pub fn add_entry(&mut self, entry: ContextEntry) {
        ...
        let stats = self.stats.entry(normalized).or_insert_with(CommandStats::default);
    ...
    pub fn summarize(&mut self) -> Option<String> {
        ...
        // Get oldest half
        let to_summarize: Vec<_> = self.history
            .iter()
            .take(self.max_size)
            .collect();
        ...
        for entry in &to_summarize {
            *intent_counts.entry(entry.parsed_intent.clone()).or_insert(0) += 1;
            if entry.success {
                success_count += 1;
            }
        }
