...
pub use types::{Entity, Entities, Confidence, ConfidenceFactor};
pub use providers::{KnownAppProvider, FileSystemProvider, CompositeProvider};
pub use cache::BrainCache;
pub use grammar::{GrammarConfig, CompiledGrammar};
...
impl Brain {
    ...
    pub fn process_with_context(&self, text: &str, success_callback: Option<bool>) -> Result<TaskPlan> {
    ...
    pub async fn process_async(&self, text: &str) -> Result<TaskPlan> {
        ...
        let enhanced_confidence = self.ranker.score(
            &parsed,
            &classified,
            &self.providers,
            Some(&*ctx_read),
        ...
        if classified.confidence < self.config.confidence_threshold {
            info!("   ⚠️  Low confidence ({:.2} < {:.2}), may need clarification",
...
mod tests {
    ...
    fn test_parse() {
        ...
        assert_eq!(parsed.entities.get("app_name"), Some(&"firefox".to_string()));
    }
