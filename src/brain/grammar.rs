...
impl GrammarConfig {
    ...
    pub fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        ...
        if !path.exists() {
            warn!(
                "Grammar file not found: {:?}, using built-in patterns",
                path
            );
        ...
        info!(
            "✅ Loaded grammar config version {} with {} intents",
            config.version,
            config.intents.len()
        );
    ...
    pub fn compile(&self) -> Result<CompiledGrammar> {
        ...
        for intent_def in &self.intents {
            ...
            for pattern_def in &intent_def.patterns {
                // Compile regex
                let regex = Regex::new(&pattern_def.pattern).map_err(|e| {
                    LunaError::CommandParsing(format!(
                        "Invalid regex pattern '{}': {}",
                        pattern_def.pattern, e
                    ))
                })?;
        ...
        // Build slot map
        let slots: HashMap<String, SlotDefinition> = self
            .slots
            .iter()
