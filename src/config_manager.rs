...
impl ConfigManager {
    ...
    fn load_layered(primary_path: &Path) -> Result<LunaConfig> {
        ...
        if primary_path.exists() {
            ...
            warn!("Primary config file not found: {:?}, using defaults", primary_path);
        }
    ...
    fn apply_env_overrides(mut config: LunaConfig) -> Result<LunaConfig> {
        ...
        if let Ok(val) = std::env::var("LUNA_AUDIO__SAMPLE_RATE") {
            ...
            info!("Applied env override: sample_rate={}", config.audio.sample_rate);
        }
        
        if let Ok(val) = std::env::var("LUNA_AUDIO__CHANNELS") {
            config.audio.channels = val.parse().map_err(|e| {
                LunaError::Config(format!("Invalid LUNA_AUDIO__CHANNELS: {}", e))
            })?;
        ...
        if let Ok(val) = std::env::var("LUNA_AUDIO__SILENCE_THRESHOLD") {
            ...
            info!("Applied env override: silence_threshold={}", config.audio.silence_threshold);
        }
        
        // System overrides
        if let Ok(val) = std::env::var("LUNA_SYSTEM__LOG_LEVEL") {
            config.system.log_level = val;
            info!("Applied env override: log_level={}", config.system.log_level);
        }
        ...
        if let Ok(val) = std::env::var("LUNA_BRAIN__CONFIDENCE_THRESHOLD") {
            ...
            info!("Applied env override: confidence_threshold={}", config.brain.confidence_threshold);
        }
        ...
        if let Ok(val) = std::env::var("LUNA_PERFORMANCE__MAX_THREADS") {
            ...
            info!("Applied env override: max_threads={}", config.performance.max_threads);
        }
    ...
    pub fn enable_hot_reload(&mut self) -> Result<()> {
        ...
        let mut watcher = notify::recommended_watcher(move |res: notify::Result<Event>| {
            ...
            LunaError::Config(format!("Failed to create file watcher: {}", e))
        })?;
        
        watcher.watch(&self.config_path, RecursiveMode::NonRecursive).map_err(|e| {
            LunaError::Config(format!("Failed to watch config file: {}", e))
        })?;
