...
pub async fn setup_logging_subscriber(event_bus: &EventBus) -> usize {
    event_bus
        .subscribe(vec![], |envelope: &EventEnvelope| match &envelope.event {
            ...
            LunaEvent::ActionCompleted {
                ...
                if *success {
                    ...
                    warn!(
                        "⚠️  Action '{}' failed after {}ms",
                        action_type, duration_ms
                    );
                }
            }
            LunaEvent::Error {
            ...
            LunaEvent::ClarificationRequested {
                command,
                confidence,
                missing_slots,
                suggestions,
                ...
                warn!(
                    "❓ Clarification needed for '{}' (confidence: {:.2})",
                    command, confidence
                );
            ...
            LunaEvent::ClarificationAnswered {
                original_command,
                clarification,
                resolved_command,
            } => {
            ...
            LunaEvent::GrammarReloaded {
                pattern_count,
                timestamp,
            } => {
                info!(
                    "🔄 Grammar reloaded: {} patterns at timestamp {}",
                    pattern_count, timestamp
                );
            }
            ...
            LunaEvent::PlanStarted {
                ...
                info!(
                    "🚀 Plan {} started: {} steps, {} parallel groups",
                    plan_id, step_count, parallel_groups
                );
            }
            LunaEvent::PlanCompleted {
                ...
                if *success {
                    info!(
                        "✅ Plan {} completed in {}ms: {} steps succeeded",
                        plan_id, total_duration_ms, steps_completed
                    );
                } else {
                    warn!(
                        "⚠️  Plan {} failed after {}ms: {} succeeded, {} failed",
            ...
            LunaEvent::ActionRetry {
                ...
                warn!(
                    "🔄 Retrying {} (attempt {}/{}): {}",
                    action_type, attempt, max_attempts, error
                );
            }
...
pub async fn setup_metrics_subscriber(event_bus: &EventBus, metrics: Arc<Metrics>) -> usize {
    event_bus
        .subscribe(vec![], move |envelope: &EventEnvelope| {
...
pub async fn setup_debug_subscriber(event_bus: &EventBus) -> usize {
    event_bus
        .subscribe(vec![], |envelope: &EventEnvelope| {
            debug!(
