...
impl CapabilityDetector {
    ...
    pub async fn detect_all(&self) -> Vec<Capability> {
        ...
        // Audio control
        capabilities.push(self.detect_tool("pactl", "Audio Control (PulseAudio)", 
            Some("sudo apt install pulseaudio-utils")).await);
        capabilities.push(self.detect_tool("amixer", "Audio Control (ALSA)", 
            Some("sudo apt install alsa-utils")).await);
        
        // Media control
        capabilities.push(self.detect_tool("playerctl", "Media Control (MPRIS)", 
            Some("sudo apt install playerctl")).await);
        
        // Brightness control
        capabilities.push(self.detect_tool("xbacklight", "Brightness Control (X11)", 
            Some("sudo apt install xbacklight")).await);
        capabilities.push(self.detect_tool("brightnessctl", "Brightness Control", 
            Some("sudo apt install brightnessctl")).await);
        
        // Screen locking
        capabilities.push(self.detect_tool("gnome-screensaver-command", "Screen Lock (GNOME)", 
            Some("Pre-installed on GNOME")).await);
        capabilities.push(self.detect_tool("xdg-screensaver", "Screen Lock (XDG)", 
            Some("sudo apt install xdg-utils")).await);
        
        // Window management
        capabilities.push(self.detect_tool("wmctrl", "Window Management", 
            Some("sudo apt install wmctrl")).await);
        capabilities.push(self.detect_tool("xdotool", "Window Management (Advanced)", 
            Some("sudo apt install xdotool")).await);
        
        // System control
        capabilities.push(self.detect_tool("systemctl", "System Control (systemd)", None).await);
    ...
    async fn detect_tool(&self, tool: &str, capability_name: &str, install_hint: Option<&str>) -> Capability {
        let version = self.get_tool_version(tool).await;
        let available = version.is_some();
        
        if !available {
            warn!("Capability '{}' not available: {} not found", capability_name, tool);
    ...
    async fn get_tool_version(&self, tool: &str) -> Option<String> {
        ...
        if Command::new("which").arg(tool).output().ok()?.status.success() {
