    ...
    pub async fn play_pause(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let status = Command::new("playerctl")
                ...
                .status()
                .map_err(|e| {
                    LunaError::SystemOperation(format!("Failed to control media: {}", e))
                })?;
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation(
                    "Failed to control Music app".to_string(),
                ))
            }
        }
    }
    ...
    pub async fn previous_track(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            let status = Command::new("playerctl")
                ...
                .status()
                .map_err(|e| {
                    LunaError::SystemOperation(format!("Failed to go to previous track: {}", e))
                })?;
    ...
    pub async fn stop(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            let status = Command::new("playerctl")
                ...
                .status()
                .map_err(|e| {
                    LunaError::SystemOperation(format!("Failed to stop playback: {}", e))
                })?;
    ...
    pub async fn get_current_track(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            let output = Command::new("playerctl")
                ...
                .output()
                .map_err(|e| {
                    LunaError::SystemOperation(format!("Failed to get track info: {}", e))
                })?;
