...
impl FileSearch {
    ...
    pub async fn search_by_name(&self, query: &str, limit: usize) -> Result<Vec<PathBuf>> {
        ...
        let mut scored_files: Vec<(PathBuf, f32)> = all_files
            ...
            .filter_map(|file_info| {
                let file_name = file_info.path.file_name()?.to_str()?;
    ...
    pub async fn find_recent(&self, limit: usize) -> Result<Vec<PathBuf>> {
        ...
        let results: Vec<PathBuf> = all_files.into_iter().take(limit).map(|f| f.path).collect();
    ...
    fn is_text_file(path: &PathBuf) -> bool {
        let text_extensions = [
            "txt", "md", "rs", "py", "js", "ts", "jsx", "tsx", "html", "css", "json", "toml",
            "yaml", "yml", "xml", "c", "cpp", "h", "hpp", "java", "go", "sh", "bash",
        ];
