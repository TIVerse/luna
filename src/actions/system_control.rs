...
impl SystemControl {
    ...
    pub async fn set_volume(&self, level: u8) -> Result<String> {
        ...
        if level > 100 {
            return Err(LunaError::InvalidParameter("Volume must be 0-100".to_string()));
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation("Failed to set volume".to_string()))
            }
        }
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation("nircmd not found. Please install nircmd".to_string()))
            }
        }
    ...
    pub async fn adjust_volume(&self, delta: i8) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let status = Command::new("pactl")
                .args(&["set-sink-volume", "@DEFAULT_SINK@", &format!("{}{}%", sign, delta)])
                .status()
                .map_err(|e| LunaError::SystemOperation(format!("Failed to adjust volume: {}", e)))?;
            
            if status.success() {
                Ok(format!("Volume adjusted by {}", delta))
            } else {
                Err(LunaError::SystemOperation("Failed to adjust volume".to_string()))
            }
        }
        
        #[cfg(not(target_os = "linux"))]
        {
            Err(LunaError::SystemOperation("Volume adjustment not implemented for this platform".to_string()))
        }
    }
    ...
    pub async fn set_brightness(&self, level: u8) -> Result<String> {
        ...
        if level > 100 {
            return Err(LunaError::InvalidParameter("Brightness must be 0-100".to_string()));
        }
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let status = Command::new("brightnessctl")
                ...
                .status()
                .map_err(|e| LunaError::SystemOperation(format!("Failed to set brightness: {}", e)))?;
            
            if status.success() {
                Ok(format!("Brightness set to {}%", level))
            } else {
                Err(LunaError::SystemOperation("Failed to set brightness. Install xbacklight or brightnessctl".to_string()))
            }
        }
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            Err(LunaError::SystemOperation("Brightness control not supported on Windows".to_string()))
        }
        ...
        #[cfg(target_os = "macos")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation("brightness command not found".to_string()))
            }
        }
    }
    ...
    pub async fn lock_computer(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            let lock_commands = ["gnome-screensaver-command -l", "xdg-screensaver lock", "loginctl lock-session"];
            ...
            Err(LunaError::SystemOperation("Failed to lock computer. No screen locker found".to_string()))
        }
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation("Failed to lock computer".to_string()))
            }
        }
    ...
    pub async fn toggle_mute(&self) -> Result<String> {
        ...
        #[cfg(target_os = "linux")]
        {
            ...
            if status.success() {
                ...
                Err(LunaError::SystemOperation("Failed to toggle mute".to_string()))
            }
        }
        
        #[cfg(not(target_os = "linux"))]
        {
            Err(LunaError::SystemOperation("Mute toggle not implemented for this platform".to_string()))
        }
    }
}
