...
pub async fn create_directory(path: &Path) -> Result<()> {
    ...
    fs::create_dir_all(path).await.map_err(|e| {
        LunaError::SystemOperation(format!("Failed to create directory {:?}: {}", path, e))
    })?;
...
mod tests {
    ...
    async fn test_move_file_directory() {
        ...
        tokio::fs::write(src_dir.join("file1.txt"), b"content1")
            .await
            .unwrap();
        tokio::fs::write(src_dir.join("file2.txt"), b"content2")
            .await
            .unwrap();
    ...
    async fn test_move_file_nested_directory() {
        ...
        tokio::fs::create_dir_all(src_dir.join("sub1").join("sub2"))
            .await
            .unwrap();
        tokio::fs::write(src_dir.join("root.txt"), b"root")
            .await
            .unwrap();
        tokio::fs::write(src_dir.join("sub1").join("file1.txt"), b"file1")
            .await
            .unwrap();
        tokio::fs::write(
            src_dir.join("sub1").join("sub2").join("file2.txt"),
            b"file2",
        )
        .await
        .unwrap();
        ...
        // Verify content
        let content =
            tokio::fs::read_to_string(dst_dir.join("sub1").join("sub2").join("file2.txt"))
                .await
                .unwrap();
    ...
    async fn test_move_file_with_unicode_name() {
        ...
        tokio::fs::write(&src, "Unicode content: 你好世界")
            .await
            .unwrap();
