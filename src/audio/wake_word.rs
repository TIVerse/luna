...
impl WakeWordDetector {
    ...
    pub fn new_with_engine(engine: WakeWordEngine, keywords: Vec<String>, sensitivity: f32) -> Result<Self> {
        match engine {
            #[cfg(feature = "porcupine")]
            ...
            WakeWordEngine::Porcupine => {
                warn!("Porcupine not available (feature disabled), falling back to energy detection");
        ...
        if let Some(ref porcupine) = self.porcupine {
            ...
            for chunk in pcm.chunks(frame_length) {
                ...
                match porcupine.process(chunk) {
                    Ok(keyword_index) => {
                        if keyword_index >= 0 {
                            info!("🎤 Wake word detected: {}", self.keywords[keyword_index as usize]);
