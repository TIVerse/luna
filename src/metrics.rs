...
impl Metrics {
    ...
    pub fn print_summary(&self) {
        ...
        if processed > 0 {
            ...
            if total_count > 0 {
                let avg_total =
                    self.total_processing_latency.load(Ordering::Relaxed) / total_count as u64;
            ...
            if audio_count > 0 {
                let avg_audio =
                    self.audio_capture_latency.load(Ordering::Relaxed) / audio_count as u64;
...
impl Drop for MetricTimer {
    fn drop(&mut self) {
        self.metrics
            .record_latency(self.phase, self.start.elapsed());
    }
}
...
pub mod prometheus {
    ...
    pub async fn start_exporter(addr: &str) -> Result<()> {
        let socket_addr: SocketAddr = addr.parse().map_err(|e| {
            crate::error::LunaError::Config(format!("Invalid metrics address: {}", e))
        })?;
        ...
        PrometheusBuilder::new()
            ...
            .install()
            .map_err(|e| {
                crate::error::LunaError::Config(format!(
    ...
    pub fn init_metrics() {
        ...
        describe_counter!(
            ...
            "Total wake words detected"
        );
        describe_counter!("luna_errors_total", "Errors by type and module");

        // Describe histograms
        describe_histogram!(
            "luna_audio_capture_duration_ms",
            Unit::Milliseconds,
            "Audio capture duration"
        );
        describe_histogram!(
            "luna_stt_duration_ms",
            Unit::Milliseconds,
            "Speech-to-text duration"
        );
        describe_histogram!(
            "luna_parsing_duration_ms",
            Unit::Milliseconds,
            "Command parsing duration"
        );
        describe_histogram!(
            "luna_execution_duration_ms",
            Unit::Milliseconds,
            "Action execution duration"
        );
        describe_histogram!(
            "luna_total_processing_duration_ms",
            Unit::Milliseconds,
